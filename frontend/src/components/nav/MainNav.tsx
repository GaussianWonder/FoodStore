        ...
        <div className="flex items-center justify-between space-x-4 lg:space-x-10">
          <div className="flex lg:w-0 lg:flex-1">
            {auth.token && (
              <Link
                to="/"
                className="px-5 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg flex items-center justify-center select-none cursor-pointer"
              >
          ...
          {auth.token && auth.user?.admin && <AdminNav />}
          {auth.token && !auth.user?.admin && <UserNav />}
          ...
          {!auth.token && (
            <div className="items-center justify-end flex-1 hidden space-x-4 sm:flex">
              <Link className="px-5 py-2 text-sm font-medium text-gray-500 bg-gray-100 rounded-lg" to="/login">
                Log in
              </Link>

              <Link className="px-5 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg" to="/register">
                Sign up
              </Link>
            </div>
          )}
